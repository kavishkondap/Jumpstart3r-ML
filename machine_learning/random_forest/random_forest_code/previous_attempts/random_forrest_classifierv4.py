...
val_predict = model_rf.predict (val_data)

print('AUC Train: {:.4f}\nAUC Valid = {:.4f}'.format(metrics.roc_auc_score(training_labels, training_predict),
                                                     metrics.roc_auc_score(val_labels, val_predict)))

val_labels_prob = model_rf.predict_proba (val_data)
...
y_valid_prob_tree = np.stack([m.predict_proba(val_data)[:,1] for m in model_rf.estimators_])
...
valid_auc_trees = [metrics.roc_auc_score(val_labels, (y_valid_prob_tree[:i+1].mean(0) > 0.5).astype(int)) for i in range(num_trees)]
